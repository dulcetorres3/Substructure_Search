    ...
    def fingerprint(self):
        ...
        for node_1 in self.nodes:
            ...
            for node_2 in self.nodes:
                ...
                for path in sorted(
                    nx.all_simple_edge_paths(self.graph, node_1, node_2, cutoff=cutoff)
                ):
                    path_string = " "
                    ...
                    for node_a, node_b in path:
                        path_string += (
                            node_a[0]
                            + node_b[0]
                            + str(self.graph[node_a][node_b]["weight"])
                        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="This script can be used to perform a substructure screen for compounds in the test_compounds subdirectory."
    )
    parser.add_argument(
        "-sdf_file",
        help="The name of the file correspnidng to a compound and a substructure in the test_compounds sub directory",
        type=str,
        nargs=2,
    )
    parser.add_argument(
        "-nodes_and_bonds",
        help="A list corresponding to the nodes and bonds of a compound and substructure. Put the list within quotations to avoid error from the commnad line",
        nargs=4,
        type=list,
    )
    parser.add_argument(
        "--hydrogens",
        help="A boolean specifying whether or not hydrogens are included in the compound and substructure",
        default=False,
        type=bool,
    )
    ...
    if args.sdf_file:
        ...
        if args.hydrogens:
            compound = MoleculeScreen(
                compound_nodes, compound_bonds, included_hydrogen=True
            )
            substructure = MoleculeScreen(
                substructure_nodes, substructure_bonds, included_hydrogen=True
            )
        else:
            compound = MoleculeScreen(compound_nodes, compound_bonds)
            substructure = MoleculeScreen(
                substructure_nodes, substructure_bonds, substructure_bonds
            )
